<!--
  Product-shot quality Vue 3 viewer for a Tripo-generated glb.
  ...
  that still reads as a real studio render:
    ...
    - PMREM env:   RoomEnvironment at 30% intensity (just enough IBL for PBR;
                     any higher and it'd wash out the colored lights)
...
const props = defineProps<{ glbUrl: string | null }>();

const canvasRef = ref<HTMLCanvasElement | null>(null);
let sceneHandle: ReturnType<typeof setupScene> | null = null;

onMounted(() => {
  if (!canvasRef.value) return;
  sceneHandle = setupScene(canvasRef.value);
  if (props.glbUrl) sceneHandle.loadModel(props.glbUrl).catch(console.error);
});

onBeforeUnmount(() => sceneHandle?.dispose());

watch(
  () => props.glbUrl,
  (url) => {
    if (url && sceneHandle) sceneHandle.loadModel(url).catch(console.error);
  }
);
 ...
 * slowly drift and breathe. Repainted each frame (throttled in the render
 * loop) so the GPU upload stays cheap.
 */
