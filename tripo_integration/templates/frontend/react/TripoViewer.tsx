// Product-shot quality React viewer for a Tripo-generated glb.
//
...
//   - Fill light:  saturated cyan, opposite side — tints the shadow side,
//                    subtly breathes in sync with the cyan pool
//   - Rim light:   hot magenta, high & behind — pops the silhouette with a
//                    neon edge, subtly breathes in sync with the magenta pool
...
//
// Background: animated twin-spotlight vignette — magenta and cyan pools
// slowly drift and pulse, giving the scene a live club-stage feel.
...
export function TripoViewer({ glbUrl, style, className }: Props) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const stateRef = useRef<ReturnType<typeof setupScene> | null>(null);

  useEffect(() => {
    if (!canvasRef.current) return;
    const state = setupScene(canvasRef.current);
    stateRef.current = state;
    return () => state.dispose();
  }, []);

  useEffect(() => {
    if (!glbUrl || !stateRef.current) return;
    let cancelled = false;
    stateRef.current.loadModel(glbUrl).catch((err) => {
      if (!cancelled) console.error("Tripo viewer: failed to load glb", err);
    });
    return () => {
      cancelled = true;
    };
...
function setupScene(canvas: HTMLCanvasElement) {
  ...
  async function loadModel(glbUrl: string) {
    ...
    normalizeAndGroundModel(gltf.scene);
...
function applyStudioLighting(scene: THREE.Scene, renderer: THREE.WebGLRenderer) {
  ...
  key.shadow.radius = 4; // soften the shadow edge
  scene.add(key);

  // Fill — saturated cyan. Tints the shadow side a cool teal, huge
  // color-contrast against the warm key. This is the backbone of the neon look.
 ...
 *
 * Returns both the texture and an `update(time)` that the render loop drives.
 */
function makeStudioBackground(): {
  ...
  function paint(t: number) {
    ...
    // Breathe the pool brightness ±18% around the base. Warm and cool pulse
